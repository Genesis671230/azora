...
const Edit = (props) => {
  ...
  const fetcEdithData = async () => {
    ...
    let response = await getApi("api/user/view/", id);
    console.log(response.data);
    setFieldValue("firstName", response.data?.firstName);
    setFieldValue("lastName", response.data?.lastName);
    setFieldValue("mailId", response.data?.mailId);
    setFieldValue("phoneNumber", response.data?.phoneNumber);

    setFieldValue("role", response.data?.role);
    setFieldValue("propertyCanAdd", response.data?.propertyCanAdd);
    setFieldValue("name", response.data?.name);
    setFieldValue("mobileNumber", response.data?.mobileNumber);
    setFieldValue("reportingTo", response.data?.reportingTo);
    setFieldValue("dob", response.data?.dob);
    setFieldValue("mailId", response.data?.mailId);
    setFieldValue("address", response.data?.address);
    setFieldValue("target", response.data?.target);
    setFieldValue("targetDate", response.data?.targetDate);
    setFieldValue("revenuePercentage", response.data?.revenuePercentage);
    setFieldValue("confirmPassword", response.data?.confirmPassword);
    setFieldValue("profilePicture", response.data?.profilePicture);
    setFieldValue(
      "profilePictureRemarks",
      response.data?.profilePictureRemarks
    );
    setFieldValue("offerLetter", response.data?.offerLetter);
    setFieldValue("offerLetterRemarks", response.data?.offerLetterRemarks);
    setFieldValue("passportCopy", response.data?.passportCopy);
    setFieldValue("passportCopyRemarks", response.data?.passportCopyRemarks);
    setFieldValue("visaPage", response.data?.visaPage);
    setFieldValue("visaPageRemarks", response.data?.visaPageRemarks);
    setFieldValue("labourCard", response.data?.labourCard);
    setFieldValue("labourCardRemarks", response.data?.labourCardRemarks);
    setFieldValue("emiratesId", response.data?.emiratesId);
    setFieldValue("emiratesIdRemarks", response.data?.emiratesIdRemarks);
    setFieldValue("status", response.data?.status);
    setFieldValue("able", response.data?.able);
    setFieldValue("doj", response.data?.doj);
    setSelectedAuthorities(response.data?.authorities)
          ...
          {/* <Grid templateColumns="repeat(12, 1fr)" gap={3}>
                        ...
                        <GridItem colSpan={{ base: 12 }}>
                            ...
                            <Input
                                ...
                                fontWeight='500'
                                borderColor={errors.mailId && touched.mailId ? "red.300" : null}
                            />
                            <Text mb='10px' color={'red'}> {errors.mailId && touched.mailId && errors.mailId}</Text>
                        </GridItem>
                        <GridItem colSpan={{ base: 12 }}>
                            <FormLabel display='flex' ms='4px' fontSize='sm' fontWeight='500' mb='8px'>
            ...
            >
              ...
              <GridItem colSpan={{ base: 12 }}>
                ...
                <Input
                  ...
                  fontWeight="500"
                  borderColor={
                    errors.mailId && touched.mailId ? "red.300" : null
                  }
                />
                <Text mb="10px" color={"red"}>
                  {" "}
                  {errors.mailId && touched.mailId && errors.mailId}
                </Text>
              </GridItem>
              <GridItem colSpan={{ base: 12 }}>
                <FormLabel
